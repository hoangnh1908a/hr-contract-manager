...
public class AuditLogService {
  ...
  public static Specification<AuditLog> filterBy(
    ...
    return (root, query, cb) -> {
      ...
      if (StringUtils.isNotBlank(auditLogRequest.getTableName())) {
        predicates.add(cb.like(cb.lower(root.get("tableName")), "%" + auditLogRequest.getTableName().toLowerCase() + "%"));
      }
      ...
      if (StringUtils.isNotBlank(auditLogRequest.getOldValue())) {
        predicates.add(cb.like(cb.lower(root.get("oldValue")), "%" + auditLogRequest.getOldValue().toLowerCase() + "%"));
      }
      if (StringUtils.isNotBlank(auditLogRequest.getNewValue())) {
        predicates.add(cb.like(cb.lower(root.get("newValue")), "%" + auditLogRequest.getNewValue().toLowerCase() + "%"));
      }
      if (StringUtils.isNotBlank(auditLogRequest.getAction())) {
        predicates.add(cb.like(cb.lower(root.get("action")), "%" + auditLogRequest.getAction().toLowerCase() + "%"));
      }
      if (userId != null) {
  ...
  public Page<AuditLog> getAuditlogs(AuditLogRequest auditLogRequest, Pageable pageable) {
    ...
    List<Integer> userIds = auditLogPage.stream().map(AuditLog::getUserId).filter(Objects::nonNull).toList();

    Map<Integer, String> userIdToFullName = userInfoRepository.findAllById(userIds).stream().collect(Collectors.toMap(UserInfo::getId, UserInfo::getFullName));

      return auditLogPage.map(e -> {
