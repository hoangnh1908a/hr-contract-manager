...
public class EmployeeService {
  ...
  public static Specification<Employee> filterBy(EmployeeRequest employeeRequest) {
    return (root, query, cb) -> {
      ...
      if (StringUtils.isNotBlank(employeeRequest.getFullName())) {
        predicates.add(
            cb.like(
                ...
                "%" + employeeRequest.getFullName().toLowerCase() + "%"));
      }
      if (StringUtils.isNotBlank(employeeRequest.getNumberId())) {
        predicates.add(
            cb.like(
                cb.lower(root.get("numberId")),
                "%" + employeeRequest.getNumberId().toLowerCase() + "%"));
      }
      ...
      if (StringUtils.isNotBlank(employeeRequest.getEmail())) {
        predicates.add(
            cb.like(
                cb.lower(root.get("email")), "%" + employeeRequest.getEmail().toLowerCase() + "%"));
      }
  ...
  private static void setUpdateEmployee(EmployeeRequest employeeRequest, Employee e) {
    ...
    if (StringUtils.isNotBlank(employeeRequest.getSex())) e.setSex(employeeRequest.getSex().trim());
    ...
    if (employeeRequest.getStatus() != null) e.setStatus(employeeRequest.getStatus());
  ...
  public List<EmployeeNameData> getAllNameEmployees() {

    return employeeRepository.findEmployeeName();
  }

  public EmployeeValueData getEmployeeById(Integer id, Locale locale) {
    EmployeeValueData employee =
        employeeRepository
            .findByIdCustom(id)
            .orElseThrow(
                () ->
    ...
    auditLogService.saveAuditLog(Constants.GET_ID, TABLE_NAME, id, "", "");
  ...
  public Employee createEmployee(EmployeeRequest nameRequest, Locale locale) {
    ...
    auditLogService.saveAuditLog(
        Constants.ADD, TABLE_NAME, employee.getId(), "", Utils.gson.toJson(employee));
    return employee;
  }
  ...
  public void deleteEmployee(Integer id, Locale locale) {
    ...
    auditLogService.saveAuditLog(
        Constants.DELETE, TABLE_NAME, id, Utils.gson.toJson(employee.get()), "");
  }
}
